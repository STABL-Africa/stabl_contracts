...
| [`stabl_threshold_policy`](contracts/stabl_threshold_policy) | M-of-N threshold policy implementing OpenZeppelin's `Policy` trait over `simple_threshold`. Deployed once per network; each account attaches it to a context rule with `{ threshold }`. Without a policy a rule requires every signer, so this is what turns two signers into 1-of-2 or three into 2-of-3. Threshold is not auto-adjusted on signer changes; clients must bundle `set_threshold` with them. |
