...
| [`stabl_account_factory`](contracts/stabl_account_factory) | Permissionless, immutable factory that deploys `stabl_passkey_multi_signer` instances from a pinned wasm hash via `deploy(salt, signers, policies)`. Exists because the account needs its constructor and some client SDKs can only issue plain invocations, not constructor deploys. `predict(salt)` gives the address ahead of time. |
